...
public enum LiftPersistenceNamespace {
    /// Stable per-app key for the one interrupted Lift session snapshot.
    public static let activeSessionKey = "noop.activeLiftSession"
}
...
enum LiftSessionPersistence {
    ...
    struct Snapshot: Codable, Equatable {
        var startSec: Int
        /// Stable database id for the session. Optional for snapshots written before Lift sessions
        /// had an explicit id; resume creates one for those sessions.
        var sessionId: String?
        /// The device/strap owner chosen by the parent repository. Optional keeps old snapshots
        /// resumable while preventing a new device from silently taking ownership of a current one.
        var deviceId: String?
    ...
    static let defaultsKey = LiftPersistenceNamespace.activeSessionKey
