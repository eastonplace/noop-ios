...
final class LiftSessionController: ObservableObject {
    ...
    /// Stable id used by the atomic store save and retained across retry attempts.
    @Published private(set) var sessionId: String?
    /// Repository-selected strap/device owner. Nil preserves compatibility for callers that do not
    /// yet have a device id, while an owned session can reject a strap switch before saving.
    ...
    enum OwnershipError: Error, Equatable {
    ...
    func start(plan: [LiftPlanItem], programId: String?, programName: String?,
               deviceId: String? = nil, sessionId: String? = nil) {
        // A finished session that failed to save still owns the controller and its persisted
        // snapshot. Refuse a second start until the parent either retries successfully or discards.
        guard engine == nil else { return }
        // New production callers pass the repository owner. Keep nil as a source-compatible escape
        // hatch for older tests and callers that intentionally exercise an unowned session.
        if let deviceId, deviceId.isEmpty { return }
    ...
    /// Validate the repository owner before building rows for `saveLiftSession`.
    /// Compatibility sessions without an owner accept the caller's device choice.
    func validateOwner(_ expected: String) throws {
        guard let deviceId else { return }
        guard deviceId == expected else {
            throw OwnershipError.ownerMismatch(expected: expected, actual: deviceId)
        }
    }
