...
final class DebouncedLogTailPersistenceTests: XCTestCase {
    ...
    func testRapidBurstBatchesAndBoundsTail() async throws {
        ...
        let persistence = DebouncedLogTailPersistence(
            debounceInterval: 0.03, tailLimit: 100,
            loadPersisted: { [] }, persist: writes.append)
        for i in 0..<500 { persistence.append("line \(i)") }
    ...
    func testForceFlushIncludesNewestQueuedMessages() async {
        let writes = Writes()
        let persistence = DebouncedLogTailPersistence(
            debounceInterval: 60, tailLimit: 10,
            loadPersisted: { [] }, persist: writes.append)
    ...
    func testSeparateRapidBurstsProduceOneWritePerBurst() async throws {
        let writes = Writes()
        let persistence = DebouncedLogTailPersistence(
            debounceInterval: 0.025, tailLimit: 100,
            loadPersisted: { [] }, persist: writes.append)
        for i in 0..<50 { persistence.append("first \(i)") }
        try await Task.sleep(for: .milliseconds(70))
        for i in 0..<50 { persistence.append("second \(i)") }
        try await Task.sleep(for: .milliseconds(70))
    ...
    func testExistingDurableTailIsPreservedAndBounded() async {
        let writes = Writes()
        let persistence = DebouncedLogTailPersistence(
            debounceInterval: 60, tailLimit: 3,
            loadPersisted: { ["old 1", "old 2"] }, persist: writes.append)
    ...
    func testClearCannotBeUndoneByPendingDebounce() async throws {
        let writes = Writes()
        let persistence = DebouncedLogTailPersistence(
            debounceInterval: 0.04, tailLimit: 10,
            loadPersisted: { [] }, persist: writes.append)
