...
final class SmartAlarmBackgroundTaskRegistrarTests: XCTestCase {
    @MainActor
    func testBackgroundRequestPersistenceRoundTripsBeforeSubmission() throws {
        let suite = "SmartAlarmBackgroundPersistenceTests-\(UUID().uuidString)"
        ...
        XCTAssertTrue(SmartAlarmRuntimeBackgroundScheduler.persistRequest(
            request,
            defaults: defaults
        ))
        XCTAssertEqual(
            SmartAlarmRuntimeBackgroundScheduler.loadRequest(defaults: defaults),
            request
        )
    }
